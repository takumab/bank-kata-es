...
class AccountEventHandler {
  ...
  constructor({eventsRepository, accountEventProcessor}: {
...
class AccountsRepository {
  ...
  async findAllBy(email: string): Promise<Array<Account>> {
    return this.accounts.filter((account: Account) => account.customer.email === email);
  }
...
class EventsRepository {
	...
	async findById(eventId: string): Promise<BaseAccountEvent | undefined> {
		return this.events.find((event: BaseAccountEvent) => event.eventId === eventId)
	}
	...
	findAllBy(accountId: string) {
		return this.events.filter((event: BaseAccountEvent) => event.payload.accountId === accountId)
	}
}
...
class AccountEventProcessor {
	...
	constructor({ eventsRepository, accountsRepository }: {eventsRepository: EventsRepository, accountsRepository: AccountsRepository}) {
		this.eventsRepository = eventsRepository;
	...
	async buildProjection(accountId: string) {
		...
		const createdAccountEvents = this.eventsRepository.findAllBy(accountId).filter(isAccountCreatedEvent)
...
const isAccountCreatedEvent = (event: BaseAccountEvent): event is AccountCreatedEvent => {
	return event.eventType === EventType.AccountCreated;
}

const eventsRepository = new EventsRepository();
const accountsRepository = new AccountsRepository();
const accountEventProcessor = new AccountEventProcessor({eventsRepository, accountsRepository});
const accountEventHandler = new AccountEventHandler({eventsRepository, accountEventProcessor});
...
describe("AccountEventHandler", () => {
  test("should save the event in events repository", async () => {
    ...
    const accountEventHandler = new AccountEventHandler(
      {eventsRepository: eventsRepository, accountEventProcessor: accountEventProcessor},
    );
